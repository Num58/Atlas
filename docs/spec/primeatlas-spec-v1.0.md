...
- **一句话描述**：PrimeAtlas 是以“成为谁”为北极星的个人成长操作系统；从身份差距推导目标、里程碑和专业计划，并根据执行、恢复、能量、成果和现实场景持续适配。
- **目标用户**：18 岁以上普通健康成年人；有身份跃迁愿望、同时经营 1–3 个活跃成长域、现实时间和能量波动明显，并重视专业性、解释性和数据主权。
- **核心问题**：现有单点工具无法把身份目标、专业知识、身体状态、有限时间和现实变化组织成连续、可信、可执行且可重校准的成长路径。
- **北极星叙事**：不是“今天做了吗”，而是“离目标身份近了吗”；不是管理任务，而是管理用户成为谁。
...
终局范围覆盖 A–K、12 类专业职责和全局数据/安全/离线能力：身份、目标、计划、执行、成果、能量、Prime Pulse、调性、数据洞察、基础能力和交叉融合。

### 2.2 首个开发版本建议：V0.2 身份—目标闭环

> 该版本只有在本文相关合同 Frozen 后才能转为 Dev Ready。

| 优先级 | 功能 | Requirement | 验收摘要 |
|---|---|---|---|
| P0 | 身份自由表达与渐进追问 | A1 | 形成可编辑草案；未确认不进入正式画像 |
| P0 | 当前/目标身份确认和版本化 | A4 | 只有用户显式确认产生正式版本；`system_auto` 仅候选 |
| P0 | 活跃成长域管理 | A2/B4 | 同时最多 3 个活跃域；未激活域零展示、零占位、零权重 |
| P0 | 域内多个目标与来源解释 | B1/B4 | 每个目标关联身份差距、基线和证据；超限只建议聚焦 |
| P0 | 里程碑 | B2 | 有起点、目标状态、周期、证据和完成规则 |
| P0 | 旅程总览与身份关联 | X2 | 在旅程/目标详情展示身份→域→目标→里程碑关系；V0.2 不创建无上游计划来源的“今日行动” |
| P0 | 本地优先与基础同步钩子 | J5/X1 | 草案、确认和编辑本地可靠保存；事件具备幂等、版本和来源 |
| P1 | 身份进度的阶段/趋势/证据视图 | A3 | 数据不足显示学习态；公式未 Frozen 前不显示精确百分比 |

### 2.3 V0.2 设计范围（Candidate 页面合同）

V0.2 最小闭环固定为：

`价值预览/游客继续 → 身份自由表达与渐进追问 → 身份草案编辑 → 显式确认 → 活跃成长域选择 → 目标候选/编辑/确认 → 里程碑确认 → 旅程总览/目标详情 → 画像版本与本地保存`

| 页面/能力 | V0.2 状态 | 核心对象/事件 | 边界 |
|---|---|---|---|
| 价值预览/游客继续 | Candidate | 只读示例；`preview_opened` | 展示真实身份→目标→证据结构，不使用抽象 Hero |
| 身份发现 | Required | `IdentityDraft`；`draft_created/discovery_answered` | 可随时停止；模型失败保留输入 |
| 身份草案与确认 | Required | `IdentityDraft/PortraitVersion`；`portrait_confirmed` | 未确认不产生正式画像 |
| 成长域选择 | Required | `ActiveDomain`；`domain_activated/paused` | 最多3活跃域，未激活域不占位 |
| 目标候选/编辑/确认 | Required | `Goal`；`goal_proposed/confirmed` | 来源身份差距可见，域内允许多个目标 |
| 里程碑确认 | Required | `Milestone`；`milestone_created/updated` | 有证据、周期和完成规则 |
| 旅程总览与目标详情 | Required | `PortraitVersion/Goal/Milestone` | 是 V0.2 闭环终点和唯一正式编辑真源 |
| 画像版本/本地保存状态 | Required | `PortraitVersion/AuditRecord/SyncOperation` | 显示已保存本机/待同步/版本冲突 |
...
- PrimeAtlas 第一方远程身份辅助在 recipient/service identity、Consent 文案/有效期、保留删除 SLA 和测试证据冻结前保持 `Spec Blocked`；V0.2 必须可选择仅本地模板模式完成完整闭环；
...
1. 身份迁移是唯一最高主轴。
2. 最多 3 个活跃成长域，每域可有多个目标；超过上限只建议聚焦。
3. 未激活域不展示、不建业务占位、不进 Agent 上下文和指标分母。
4. 普通冲突保留“一键采纳 / 我自己来”，`blocked_user=false` 只适用于普通冲突。
5. 安全否决不可被用户、调性、编排器、模型或其他 Agent 覆盖。
6. 训练方案前适用范围确认必须主动且默认未选；命中、拒绝或无法确认均不生成普通精细训练方案；确认不替代持续安全审查。
7. Pulse 可绕过、不是训练解锁条件；长期目标势能、今日准备度和启动仪式相互独立；产品建议 500ms，并提供等效入口。
8. 指标默认阶段、趋势、证据和数据充分度；条件不足不显示伪精确。
9. 融合采集与生成可分别关闭或撤回，且无惩罚。
10. 第三方模型仅获最小必要、默认去标识化数据，需独立 Consent，默认不用于训练通用模型。
11. 12 Agent 是职责、权限和审查角色，不等于 12 个模型或真人。
12. 调性确认后仅影响后续输出，历史不回写；cooldown 只提示，不得系统阻断。
...
- 所有推断/生成值具有 `source_type`：`user_declared | device_observed | imported | system_inferred | model_generated | professional_content | deterministic_rule`。
...
| `IdentityDraft` | 当前/目标身份草案、追问状态 | owner_id、status、version |
| `ActiveDomain` | 域代码、优先级、激活状态 | owner_id + status；最多3活跃 |
| `PortraitCandidate` | 系统/模型产生的画像变化候选、证据、并发状态 | owner_id、status、base_portrait_version_id |
| `PortraitVersion` | 用户确认画像版本、证据摘要 | owner_id、version、confirmation_source |
| `EvidenceLink` | 身份、目标、里程碑和候选的来源证据关联 | subject_type/id、evidence_type/id、validity |
| `Goal` | 域内目标、身份差距来源、生命周期 | domain_id、status、version |
| `Milestone` | 起点/目标、证据、周期、完成规则 | goal_id、status |
...
只有 `user_confirm` 可进入 `confirmed`；模型完成、轮数阈值和 `system_auto` 均不得确认。审计事件使用 `identity_draft_*` 与 `portrait_version_activated`。
...
恢复或重大变化必须重跑受影响的专业、安全和时间冲突评审；`completed` 只能产生画像候选，不得自动升级画像。
...
| `pending_review` | `major + all_required_reviews_passed` | `pending_confirmation` |
| `pending_confirmation` | `user_confirm` | `active` |
| `pending_confirmation` | `user_edit` | `draft` |
| `pending_confirmation` | `user_reject` | `draft/archived`（按明确动作） |
...
安全超时、依赖不可用、规则不可用或关键数据过期时，计划不得转为通过；对应 `SafetyEvent` 必须保持 `precautionary_paused` 或进入 `blocked`。`conservative` 不是失败后的默认降级状态，只能是独立生成、独立规则版本、有效适用范围和必要安全检查全部通过的新 `PlanVersion`。`safety_status != passed` 的普通精细方案禁止进入 `active/ready`。
...
`candidate → pending_confirmation`；`pending_confirmation → accepted | rejected | withdrawn | conflicted`；`conflicted → accepted | rejected | withdrawn` 仅由用户解决；激活新正式版本使旧 active 版本 `superseded`；恢复旧版本必须创建新的 `restored(active)` 版本。

`system_auto` 只能产生候选；并发候选或版本不可自动字段合并为正式画像。
...
- event_id 唯一；operation_id 表达一次业务意图；causation/correlation 串联身份→目标→计划→执行→画像因果链。
- payload 不记录健康原文、完整笔记或完整身份叙事。
...
| 领域 | 核心接口 | 状态 |
|---|---|---|
| Identity | `create/update/submit/confirm/discard_identity_draft`、`record_identity_discovery_answer`、`request/apply_assisted_discovery`、`continue_with_local_discovery`、`list/restore_portrait_version` | V0.2 Frozen 语义；第一方远程实现仍受 Readiness 门禁 |
| Domain/Goal/Journey | `activate/pause/archive_domain`、`create_goal_candidate`、`confirm/update/pause/archive_goal`、`request/confirm_goal_recalibration`、`upsert_milestone`、`get_journey_overview/get_goal_detail` | 状态与产品语义 Frozen；最终请求/响应、错误恢复仍按测试追踪 Gap 冻结 |
| Eligibility/Consent | `present/submit_eligibility`、`grant/deny/revoke_consent`、`get_visibility_manifest` | 后续版本；签名 Blocked |
| Plan/Safety | `request_plan_generation`、`get_plan_job`、`decide_plan_change`、`record_conflict_decision`、`request_safety_reassessment` | 后续版本；签名 Blocked |
...
| Portrait/Fusion | `list/confirm/reject_portrait_candidate`、`evaluate_knowledge_source`、`decide_knowledge_suggestion` | 画像V0.2，融合后续 |
| Sync | `enqueue/get_sync_status`、本地测试适配器 `ack/retry/resolve_sync_conflict` | V0.2本地 outbox/Noop 语义；正式云 pull/push 为 Won't Have |

### 7.4 Repository 合同

所有 Repository 以 owner_id 隔离，默认排除 tombstone，审计查询可读取历史版本。

V0.2 必须冻结：
- `SubjectRepository`: current/bind_guest/switch_subject；
- `IdentityRepository`: get/save draft、active portrait、versions、append candidate、activate version；
...
- `SyncRepository`: enqueue/next_batch/mark_synced/conflict/cursor；
- `TombstoneRepository`: create/list pending。

后续版本 Repository 以 Architecture Input §4.3 为规范性附录。

### 7.5 V0.2 原子事务与依赖边界

- 身份确认必须在单一 sqlite3 事务内原子写入 `IdentityDraft.confirmed + PortraitVersion.active + AuditRecord + SyncOperation(local outbox)`；任何一步失败全部回滚，不得出现半版本或无 outbox 的“成功”。
- V0.2 实际依赖仅包括 sqlite3、设备端本地规则/模板、可选 PrimeAtlas 第一方身份辅助、SyncAdapter 本地 outbox/Noop、系统时钟/时区。
...
`eligibility_required`、`eligibility_not_supported`、`consent_required`、`consent_revoked`、`safety_blocked`、`safety_review_unavailable`、`professional_review_incomplete`、`data_insufficient`、`version_conflict`、`idempotency_key_reused`、`invalid_state_transition`、`validation_failed`、`account_ownership_mismatch`、`offline_pending`、`third_party_payload_blocked`、`dependency_unavailable`、`content_unavailable`、`not_found`。

V0.2 已在 `v0.2-dependency-degradation-matrix.md` 冻结适用依赖的 retryability、替代、审计与故障注入；但 `archive_domain`、`update_goal`、Journey read API、tombstone 防复活窗口/确定性拒绝码等逐 Use-case 恢复合同仍须在 Release Spec 归零。
...
- 今日：把身份和目标带入现实行动；
- 旅程：当前身份 → 目标身份、目标/里程碑、成果证据与重校准；
- 成长域：最多 3 个活跃域的专业工作空间；早期以体能深扎；
- 我的：画像版本、授权、数据、调性、同步、导出删除和账号治理。

### 8.2 Candidate 对象唯一归属

| 对象/能力 | 主归属候选 | 交叉入口 | 禁止重复 |
|---|---|---|---|
| 当前身份→目标身份 | 旅程 | 今日作用说明、成长域关联 | 不藏在“我的” |
...
| 画像治理、权限、账号 | 我的 | 旅程提供画像版本入口 | “我的”不承担身份迁移主叙事 |
...
- 禁止 Pulse 解锁、融合任务中心、同时多任务、知识静默入计划、画像候选自动成为事实；
...
第三方模型默认不可见：健康/伤病原文、完整笔记/书摘、完整身份叙事、画像历史全集、Consent 回执全文、Audit 日志和 Agent 私有推理链。
...
| 已确认身份最小摘要 | 是 | 同步启用时 | 条件可见 | 是 | 仅当前任务必要摘要，不发完整叙事 |
...
| 画像候选/历史全集 | 是 | 同步启用时 | 否 | 否 | 只发已确认当前版本最小摘要 |
...
6. **用户显式编辑**：优先于未确认普通建议；
7. **画像/计划**：画像冲突保留候选；计划版本并存并建立 supersedes；
...
| 身份/目标引导单轮 | `≤10s` | 保留输入、手动编辑 | 伪装完成的正式画像 | 否 |
...
| 编号 | Given | When | Then |
|---|---|---|---|
| AC-01 | 用户创建身份草案 | 未确认、拒绝或取消 | 不产生正式画像；输入可恢复；无惩罚 |
| AC-02 | 用户确认身份 | 创建正式画像版本 | `confirmation_source=user_explicit`，历史版本保留 |
...
| AC-07 | 普通目标/时间冲突 | 用户查看裁决 | 同时提供“一键采纳 / 我自己来”，不删除目标 |
| AC-08 | 安全规则命中 | 用户请求继续原危险方案 | 无继续入口；显示依据、替代、暂缓和复评条件 |
...
| AC-16 | 指标数据不足 | 用户查看身份/恢复/融合 | 显示阶段、证据和充分度，不显示伪精确百分比 |
| AC-17 | 模型/安全依赖超时 | 请求安全相关放行 | 保持 `precautionary_paused/blocked`，不默认通过；仅独立审核通过的新安全替代版本可执行 |
| AC-18 | 用户使用读屏、动态字体或 reduced motion | 完成核心路径 | 功能等价，无手势唯一入口和颜色唯一信息 |
...
5. 全对象物理 Schema、索引和迁移；业务删除 vs 归档、历史保留、tombstone 与防复活语义必须在 Release Spec 冻结，不得下放为实现自由；
...
2. 架构师冻结 `UC-010 archive_domain`、`UC-013 update_goal`、`UC-019 journey read` 的请求/响应与错误恢复，以及V0.2对象物理Schema、索引、迁移、事务和版本兼容；
